...
class BotaoCampo(private val campo: Campo) : JButton() {
    ...
    private fun aplicarEstilo(campo: Campo, evento: CampoEvento) {
        when (evento) {
            CampoEvento.EXPLOSAO -> this.aplicarEstiloExplodido()
            CampoEvento.ABERTURA -> this.aplicarEstiloAberto()
            CampoEvento.MARCACAO -> this.aplicarEstiloMarcado()
            else -> this.aplicarEstiloPadrao() // CampoEvento.DESMARCACAO e CampoEvento.REINICIALIZACAO
        }
